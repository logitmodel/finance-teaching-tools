...
def show_company(code):
    ...
    # 診斷
    latest = {**df.iloc[-1].to_dict(), **val_info}
    st.markdown("**最新一期診斷**")
    for msg in diagnose(latest):
        st.markdown(f"- {msg}")
...
def main():
    ...
    st.info("資料來源: Yahoo Finance 公開財報資料。台股代號格式需加 .TW(上市)或 .TWO(上櫃),例如 2330.TW")
